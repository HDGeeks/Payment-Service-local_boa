...
def creatOrder(amount, title, currency, merch_order_id):
    ...
    module = CreateOrderService(req, env("baseUrl"), env("fabricAppId"),
                                env("appSecret"), env("merchantAppId"), env("merchantCode"),)
...
class SuperappPayViewSet(ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if request.data['payment_method'] == "telebirr_superApp":
            try:
                ...
                if order['result']["result"] == 'SUCCESS':
                    ...
                    if serializer.is_valid(raise_exception=True):
                        serializer.save()
                        return Response({"rawRequest": rawRequest, "data": serializer.data})

                return Response({"message": order['result'], 'status': status.HTTP_400_BAD_REQUEST, })
            ...
            return Response({'msg': ' payment method is not telebirr super app', 'status': 'status.HTTP_400_BAD_REQUEST'})
            ...
            if verified_data:
                ...
                if order_status == "Completed":
                    ...
                    update_data = Superapp_Payment_info.objects.filter(
                        prepay_id=prepay_id).update(payment_state="completed")
                ...
                if order_status == "Paying":
                    ...
                    update_data = Superapp_Payment_info.objects.filter(
                        prepay_id=prepay_id).update(payment_state="Paying")
                ...
                if order_status == "Pending":
                    ...
                    update_data = Superapp_Payment_info.objects.filter(
                        prepay_id=prepay_id).update(payment_state="Pending")
                ...
                if order_status == "Failure":
                    ...
                    update_data = Superapp_Payment_info.objects.filter(
                        prepay_id=prepay_id).update(payment_state="Failure")
...
def queryOrder(merch_order_id):
    ...
    module = QueryOrderService(req, env("baseUrl"), env("fabricAppId"),
                               env("appSecret"), env("merchantAppId"), env("merchantCode"))
...
class CheckPaymentViewSet(ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if merch_order_id:
            ...
            merch_order_id = payment_obj.values("merch_order_id")[
                0]["merch_order_id"]
            payment_status = payment_obj.values("payment_state")[
                0]["payment_state"]
            ...
            if not payment_status == "completed":
                try:
                    ...
                    if query['result'] == 'SUCCESS':
                        ...
                        return Response({'STATUS': query["biz_content"]["order_status"], "response": query})

                    return Response({"message": query['result'], 'status': status.HTTP_400_BAD_REQUEST, })
            ...
            return Response({'msg': ' enter valid query parameter (orderId)', 'status': 'status.HTTP_400_BAD_REQUEST'})
