...
gcloud sql databases create analytics-database --instance kinmusic-postgresql-v14

analytics_database_admin_password="$(cat /dev/urandom | LC_ALL=C tr -dc 'a-zA-Z0-9' | fold -w 30 | head -n 1)"
gcloud sql users create analytics_database_admin --instance kinmusic-postgresql-v14 --password $analytics_database_admin_password
...
echo DATABASE_URL=\"postgres://analytics_database_admin:${analytics_database_admin_password}@//cloudsql/${PROJECT_ID}:${REGION}:kinmusic-postgresql-v14/analytics-database\" > .env
